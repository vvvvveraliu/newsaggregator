...
def to_bag_of_words(all_data, cache=True):
    """
    "Bag Of Words" method.
    """
    bow_path_x = 'bow_x.npy'
    bow_path_y = 'bow_y.npy'
    if cache and os.path.exists(bow_path_x) and os.path.exists(bow_path_y):
...
def to_id_list(all_data, cache=True):
    """
    "ID List" method.
    """
    MAX_LENGTH = 21
    ...
    if cache and os.path.exists(idlist_path_x) and os.path.exists(idlist_path_y):
...
if __name__ == '__main__':
    ...
    dataset = split_dataset(to_word_vector(
        all_data[:1000], '/Users/lovever/Downloads/glove'))
